...
Nous avons contraint l'utilisateur à n'utiliser que des `NUMBER` (nombres littéraux) pour la déclaration de la taille d'un tableau. En effet, dans le cas d'une expression (comme `x + z` par exemple), la valeur réelle est stockée dans le code assembleur et n'est connue qu'à l'exécution, ce qui rend impossible la vérification de la cohérence de la taille pendant la phase de compilation en Python.
