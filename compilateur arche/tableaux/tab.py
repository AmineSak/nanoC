...
cpt = 0
# Grammar including arrays and variables
# Supports: int[N] arr; int[N] arr = {vals}; int x; int x = expr;
g = Lark(r"""
...
# Map basic ops to ASM
op2asm = {"+": "add rax, rbx", "-": "sub rax, rbx"}

# Generate ASM for expressions
...
# Generate ASM for commands

def asm_commande(c):
    global cpt
    if c.data == "decl":  # array declaration
        length = c.children[0]
        name   = c.children[1].value
        return (
    ...
    if c.data == "declinit":  # array init
        ...
        return code
    if c.data == "vardecl":  # integer variable declaration
        # only reserve storage; actual directive in .data
        return ""
    if c.data == "vardeclinit":  # int x = expr
        name = c.children[0].value
        expr = c.children[1]
        return (
...
# Assemble whole program

def asm_program(p):
    # load ASM template
    with open("moule_tab.asm") as f:
        template = f.read()
    ...
    return template.replace("COMMANDE", cmd_code)
