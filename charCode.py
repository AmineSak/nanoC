...
commande: commande (";" commande)*                                  -> sequence
        ...
        | "char" IDENTIFIER "=" CHAR                               -> char_decl
        | "char" IDENTIFIER                                        -> char_decl_empty
...
def get_vars_commande(c):
    ...
    if hasattr(c, 'data'):
        ...
        if c.data == "char_decl":
            declared_chars.add(c.children[0].value)
        
        elif c.data == "char_decl_empty":
            declared_chars.add(c.children[0].value)

        elif c.data == "string_decl":
    ...
    if isinstance(node, Tree):
        ...
        if node.data == "string_decl":
            ...
            if isinstance(token, Token) and token.type == "STRING":
                acc.add(token.value)
        if node.data == "char_decl":
            token = node.children[1]
            if isinstance(token, Token) and token.type == "CHAR":
                acc.add(token.value)
...
def is_char_expr(e) -> bool:
    if isinstance(e, Token) and e.type == "CHAR":
        return True

    if isinstance(e, Token) and e.type == "IDENTIFIER":
        return e.value in declared_chars    
...
def asm_expression(e):
    ...
    if isinstance(e, Tree) and e.data == "str_concat":
        ...
        if is_char_expr(e.children[0]) and is_char_expr(e.children[1]):
...
def asm_commande(c):
    ...
    if c.data == "char_decl":
        ...
        char_token = c.children[1]
        val = ord(char_token.value[1])
        return f"mov byte [{var}], {val}"

    if c.data == "char_decl_empty":
        var = c.children[0].value
        return f"mov byte [{var}], 0"
...
mov [{var}], rax"""
    ...
    if c.data == "affectation":
        ...
        if var.value in declared_chars:
            
            return f"""{code_exp}
    mov byte [{var.value}], al"""
        else:
            return f"""{code_exp}
    mov [{var.value}], rax"""
...
mov [rax], rcx"""
    ...
    if c.data == "print":
        ...
        is_char = (
            isinstance(raw, Token) and raw.type == "CHAR"
            or (isinstance(raw, Tree) and raw.data == "str_index")
            or (isinstance(raw, Token) and raw.type == "IDENTIFIER" and raw.value in declared_chars)
        )
